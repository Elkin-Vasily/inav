...
{
    const bool allowRawAccess = (dev->flags & DEVFLAGS_USE_RAW_REGISTERS);
    return i2cExtWriteBuffer(dev->busdev.i2c.i2cBus, dev->busdev.i2c.address, reg, length, data, allowRawAccess);
}

bool i2cBusExtWriteRegister(const busDevice_t * dev, uint16_t reg, uint8_t data)
{
    const bool allowRawAccess = (dev->flags & DEVFLAGS_USE_RAW_REGISTERS);
    return i2cExtWrite(dev->busdev.i2c.i2cBus, dev->busdev.i2c.address, reg, data, allowRawAccess);
}

bool i2cBusExtReadBuffer(const busDevice_t * dev, uint16_t reg, uint8_t * data, uint8_t length)
{
    const bool allowRawAccess = (dev->flags & DEVFLAGS_USE_RAW_REGISTERS);
    return i2cExtRead(dev->busdev.i2c.i2cBus, dev->busdev.i2c.address, reg, length, data, allowRawAccess);
}

bool i2cBusExtReadRegister(const busDevice_t * dev, uint16_t reg, uint8_t * data)
{
    const bool allowRawAccess = (dev->flags & DEVFLAGS_USE_RAW_REGISTERS);
    return i2cExtRead(dev->busdev.i2c.i2cBus, dev->busdev.i2c.address, reg, 1, data, allowRawAccess);
}
